...
@pytest.fixture(
    params=[
        ...
        (
            TEST_DATA_DIR / "menu_holiday.html",
            1,
        ),  # Was a holiday menu with only one item
        (
            TEST_DATA_DIR / "menu_format_matcard.html",
            4,
        ),  # New HTML format (product-card)
        (TEST_DATA_DIR / "menu_with_co2.html", 4),  # With new CO2 footprint data
    ],
    ids=[
        "default_format",
        "holiday_menu",
...
class TestReadMenus:
    ...
    def test_read_menus_file_exists(self, html_test_files):
        ...
        assert html_test_files[
            "path"
        ].exists(), f"Test file not found: {html_test_files['path']}"
    ...
    def test_co2_footprint_values(self, html_test_files):
        """Test that co2_footprint values are valid numbers or None."""
        df = read_menus(html_test_files["path"], date=date(2025, 8, 1))
        ...
        if html_test_files["file_name"] == "with_co2.html":
            # This file is expected to have no CO2 footprint data
            assert not df["co2_footprint"].isnull().all(), "Expected some co2_footprint"
